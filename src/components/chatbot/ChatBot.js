  ...
  componentDidMount() {
    ...
    recognition.onend = function (event) {
      ...
      // 입력인 경우
        ...
        console.log("===== 서비스 선택 =====");
        if (inputOption == inputService) {
          // 음성서비스 입력
          if (result_remove_blank.includes("음성서비스")) {
            // 음성서비스 사용설정
            ...
            // 헤드트레킹 다운로드 기능 추가
            return;
          } else if (result_remove_blank.includes("이용안함")) {
            // 음성서비스 미사용설정
            localStorage.setItem("useVoiceService", false);
            inputOption = null;
            recognition.stop();
    ...
    $(function () {
      ...
      if (useVoiceService == null) {
        // 선택한 서비스가 없는 경우 서비스선택요청
        speak(
          "어떤 서비스를 이용하시겠습니까? 일번 음성서비스, 이번 헤드트래킹, 삼번 이용안함 중 선택해주세요.",
          true,
          inputService
        );
        // setTimeout(function() {
